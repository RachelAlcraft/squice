"""
This is coped from another library and is being ported to this library in the form of
MtxInterpolator in the appropriate format for this library.
The original library will be deprecated.
...
class Interpolator(ABC):
    ...
    def closest(self, point):
        al, au = math.floor(round(point.A, self.round)), math.ceil(
            round(point.A, self.round)
        )
        bl, bu = math.floor(round(point.B, self.round)), math.ceil(
            round(point.B, self.round)
        )
        cl, cu = math.floor(round(point.C, self.round)), math.ceil(
            round(point.C, self.round)
        )
        a, b, c = 0, 0, 0
    ...
    def extend_vals_with_buffer(self, buffer):
        ...
        padded = np.pad(
            self._npy, [(buffer, buffer), (buffer, buffer), (buffer, buffer)]
        )
        # print("-----------------------")
        # print(padded)
        # pad z
...
class Nearest(Interpolator):
    ...
    def get_value(self, x, y, z):
        ...
        u_f, u_m, u_s = self.get_adjusted_fms(
            closest_pnt.A, closest_pnt.B, closest_pnt.C
        )
...
class Numpest(Interpolator):
    ...
    def get_value(self, x, y, z):
        ...
        fn = RegularGridInterpolator(
            (self.x_volume, self.y_volume, self.z_volume), self._npy, method="nearest"
        )
...
class Linear(Interpolator):
    ...
    def get_value(self, x, y, z):
        u_f, u_m, u_s = self.get_adjusted_fms(x, y, z)
        fn = RegularGridInterpolator(
            (self.x_volume, self.y_volume, self.z_volume), self._npy
        )
        volume_needed = fn(np.array([u_f, u_m, u_s]))[0]
        return volume_needed
    ...
    def get_values(self, xyz):
        xyz = self.adj_values_list(xyz)
        fn = RegularGridInterpolator(
            (self.x_volume, self.y_volume, self.z_volume), self._npy, method="linear"
        )
        volume_needed = fn(np.array(xyz))
        return volume_needed
    ...
    *******************************************************************************
    ...
    def make_periodic_coeffs(self):
        ...
        if not self.padded:
            ...
            self.create_coeffs(
                self._F + self.buffer * 2,
                self._M + self.buffer * 2,
                self._S + self.buffer * 2,
            )
            # print(self._coeffs)
        ...
        if thisF > 1:
            for y in range(thisM):
                for z in range(thisS):
                    ...
                    line = self.convert_to_interp_coeffs(
                        pole, num_poles, thisF, row, lmbda
                    )
        ...
        if thisM > 1:
            for x in range(thisF):
                for z in range(thisS):
                    ...
                    line = self.convert_to_interp_coeffs(
                        pole, num_poles, thisM, row, lmbda
                    )
        ...
        if thisS > 1:
            for x in range(thisF):
                for y in range(thisM):
                    ...
                    line = self.convert_to_interp_coeffs(
                        pole, num_poles, thisS, row, lmbda
                    )
    ...
    def initial_anticausal_coeffs(self, vals, length, pole):
        ...
        if length < 2:
            ...
            return (pole / (pole * pole - 1.0)) * (
                pole * vals[length - 2] + vals[length - 1]
            )
    ...
    def applyValue7(self, val, idc, weight_length):
        ...
        ws[1] = (
            120.0 / 7.0
            + w * (-56.0 + w * (72.0 + w * (-40.0 + w2 * (12.0 + w * (-6.0 + w)))))
        ) / 720.0
        ws[2] = (
            397.0 / 7.0
            - w
            * (
                245.0 / 3.0
                + w
                * (-15.0 + w * (-95.0 / 3.0 + w * (15.0 + w * (5.0 + w * (-5.0 + w)))))
            )
        ) / 240.0
    ...
    def applyValue9(self, val, idc, weight_length):
                ...
                * (
                    ...
                    * (
                        ...
                        * (
                            308.0
                            + w
                            * (-84.0 + w * (-56.0 / 3.0 + w * (24.0 + w * (-8.0 + w))))
                        )
                    )
                )
            )
                ...
                * (
                    ...
                    * (
                        ...
                        * (
                            112.0
                            + w
                            * (-119.0 + w * (56.0 / 3.0 + w * (14.0 + w * (-7.0 + w))))
                        )
                    )
                )
            )
            ...
            * (
                ...
                * (
                    ...
                    * (
                        -134.0
                        + w
                        * (48.0 + w * (51.0 + w * (-4.0 + w) * (-1.0 + w) * (2.0 + w)))
                    )
                )
            )
        ) / 4320.0
        ...
        ws[8] = (
            1.0 - ws[0] - ws[1] - ws[2] - ws[3] - ws[4] - ws[5] - ws[6] - ws[7] - ws[9]
        )
        return ws
