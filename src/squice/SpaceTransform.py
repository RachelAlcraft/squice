"""
This class handles transformations in 3d space from a
plane defined by 3 given points to the origin
...
class SpaceTransform(object):
    def __init__(self, central, linear, planar):
        # PUBLIC INTERFACE
        self.central = v3.VectorThree(abc=central)
        self.linear = v3.VectorThree(abc=linear)
        self.planar = v3.VectorThree(abc=planar)
    ...
    def _rotate_quadrant(self, x, y, angle):
        if abs(angle) > 0.001:
            ...
            if mag > 0.0001:
                ...
                # this is the angle made with the x-axis from the original vector
                angleB = angleA - angle
                # this is the angle made with the x-axis with the rotated vector
                qEnd = qStart
                if qStart == 1:
                    if angle > angleA:
                        angleB = angle - angleA
